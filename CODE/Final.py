...
unm = "coffeemug19977@gmail.com"                        # Login credentials of our email id
pwd = "coffeemugisgreat"

r= sr.Recognizer()

engine = pyttsx3.init()                                 # Defining an engine for text to speech INIT INITAILIZES ATTRIBUTES 
...
def listen():                                                                  # function for speech to text py audio 
    with sr.Microphone() as source:
        r.adjust_for_ambient_noise(source)
        str = "Speak Now:"
        speak(str)
        audio = r.listen(source)                                                                         #audio map spectrum 
        try:
...
def sendmail():                                                                                     # Function to send email
    ...
    server = smtplib.SMTP_SSL("smtp.gmail.com", 465)                                   #makes is local host and connect it throught secure socket layer 
...
def readmail():                                                                       #Function for Reading email from Inbox easyimap is used reading emails 
...
while(1):
    ...
    if (ch == 'send') :
        ...
        str = "You have chosen to exit, Thank you"
        speak(str)
        exit(1)
